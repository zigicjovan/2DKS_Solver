...
SBATCH_TIME = "02-23:59"  # requested time limit (D-HH:MM) 
RUN_ARRAY_NAME = f"long-{K_start}-{ell1_start:.2f}_{K_end}-{ell1_end:.2f}.sh"
        ...
        for ell1 in ell1_range:
            # for ell2 in ell2_range:
                ...
                for T in T_range:
                    ...
                    mem_req = 4 * MPIrank #int(np.maximum( 4 , mem_est ))
                    mem = f"{mem_req}G" 
                    addmemcount = mem_req
...
def main():
    ...
    print("File count check: find . -mindepth 2 -type f -printf '%h\n' | sed 's#^\(\./[^/]*\)/.*#\1#' | sort | uniq -c | sort -nr")
