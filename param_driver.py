...
SBATCH_TIME = "01-00:00"  # requested time limit (D-HH:MM) 
...
def generate_tasks():
    ...
    dt_choice = [ 1e-4, 5e-5, 1e-5, 5e-6, 1e-6, 5e-7, 3e-7, 2e-7, 9e-8, 3e-8, 3e-8, 3e-8, 3e-8, 3e-8, 2e-9, 1e-9, 5e-10 ]
    rk_choice = [ 2,    4,    6,    8,    12,   16,   20,   24,   28,   32,   48,   64,   96,   128,  160,  192,  256  ]
    K_ref = 3.5
    T_width = round(0.1, 2)
    T_step = round(0.05, 2)
    ...
    for K in K_range:
        for ell1 in ell1_range:
            # for ell2 in ell2_range:
                ...
                tempidx = 8
                N = 640 #N_choice[tempidx]
                dt = 5e-6 #dt_choice[tempidx]
                MPIrank = 40 #max(1, N // 32) # 192 cores per node on Nibi
                nodes = math.ceil(MPIrank / 192)

                for T in T_range:
                    mem_est = 1 * 8 * np.ceil((1.1 * np.exp( -4.789714989 + 0.83721882 * np.log10((N**2) * (10.0**T) / dt) - 1.70503490 * elltemp
                                                + 0.23432402 * K + 0.18691756 * elltemp * np.log10((N**2) * (10.0**T) / dt) )) / 8.0 )
                    mem_req = int(np.maximum( 4 , mem_est ))
                    mem = f"{mem_req}G" 
                    addmemcount = mem_req
...
def write_run_array_sh(groups, tag, out_fname='run_array.sh', max_concurrent=0):
    ...
    for (mem, mpi_ranks, nodes), items in groups.items():
        ...
        if SEQUENTIAL_TASKS:
            ...
            lines.append(f"    prev_jobid=$(sbatch --parsable --account=rrg-bprotas --mail-user=zigicj@mcmaster.ca --mail-type=ALL \\")
            ...
            lines.append("  else")
            lines.append(f"    prev_jobid=$(sbatch --parsable --dependency=afterany:${{prev_jobid}} --account=rrg-bprotas --mail-user=zigicj@mcmaster.ca --mail-type=ALL \\")
            lines.append(f"                     --job-name={RUN_ARRAY_NAME} --nodes={nodes} \\")
            lines.append(f"                     --ntasks={mpi_ranks} --cpus-per-task=1 --time={SBATCH_TIME} --mem-per-cpu={mem_per_cpu_mib}M \\")
            lines.append(f"                     --array={array_spec} --output=slurm_logs/slurm-%A_%a.out --error=slurm_logs/slurm-%A_%a.err \\")
            lines.append(f"                     --export=ALL,PARAM_FILE={param_file} ./run_task_array.sh)")
