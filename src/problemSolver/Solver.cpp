...
void Solver::checkSpectralResolution(const SolutionData& state) {
    const size_t N1 = _params.getGridSize1();
    const size_t N2 = _params.getGridSize2();
    const size_t localN2 = _mpi.getLocalGridSize2();
    const size_t localStart2 = _mpi.getLocalGridStart2();
    const size_t cutoff1 = N1 / 3;
    const size_t cutoff2 = N2 / 3;
    constexpr double tailFraction = 0.8;
    constexpr double tolerance = 1e-10;
    double localTotalEnergy = 0.0;
    double localTailEnergy = 0.0;

    for (size_t iLocal = 0; iLocal < localN2; ++iLocal) {
        const size_t i = localStart2 + iLocal;
        const size_t mode2 = min(i, N2 - i);

        for (size_t j = 0; j < N1; ++j) {
            const size_t mode1 = min(j, N1 - j);
            const size_t k = iLocal * N1 + j;

            if (mode1 > cutoff1 || mode2 > cutoff2) {
                continue;
            }

            const double coefficientEnergy = norm(state[k]);
            localTotalEnergy += coefficientEnergy;
            const double normalizedMode1 = static_cast<double>(mode1) / cutoff1;
            const double normalizedMode2 = static_cast<double>(mode2) / cutoff2;

            if (max(normalizedMode1, normalizedMode2) >= tailFraction) {
                localTailEnergy += coefficientEnergy;
            }
        }
    }

    double localValues[2] = {localTotalEnergy, localTailEnergy};
    double globalValues[2] = {};
    MPI_Allreduce(localValues, globalValues, 2, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    const double tailRatio = globalValues[0] > 0.0 ? globalValues[1] / globalValues[0] : 0.0;

    if (_mpi.isRoot()) {
        cout << tailRatio << flush;

        if (tailRatio > tolerance) {
            cerr << "\nWARNING: Fourier spectrum may be under-resolved.\n";
        }
    }
}
...
void Solver::findContinuationForInitialData(SolutionData& vTargetState) {
    ...
    for (const auto& testcaseEntry : filesystem::directory_iterator(dirDataRoot)) {
        ...
        if (timeBegin == string::npos) {
            continue;
        }
        
        const size_t valueBegin = timeBegin + 3;
        const size_t valueEnd = testcaseName.find("_opt_", valueBegin); 
    ...
    if (!continuedFile.empty()) {
        const filesystem::path originalInitialDataFile = _paths.getInitialDataFile();
        _paths.setInitialDataFile(continuedFile);
        vTargetState.loadData(InitialState);
        _paths.setInitialDataFile(originalInitialDataFile);
        ...
        if (_mpi.isRoot()) {
            cout << "No saved continuation data; generated initial guess.\n" << flush;
        }
    }
}
