function [ v_out , u_out , utilityout ] = solve_2DKS_matlab(IC,solver,N,K,L_s1,L_s2,dt,T,save_each,Ntime_save_max,utility1,utility2)
    ...
    switch solver 
        ...
        case {'LLE'}                                                                       
            ...
            for i = 2:Ntime
                ...
                % nonlinear terms and solution substeps
                for k = 1:4
                    v_step2x = reshape( D1vec .* v_stepp, [ N , N_x2 ] );                     % f_x
                    v_step2y = reshape( D2vec .* v_stepp, [ N , N_x2 ] );                     % f_y
                    w1_r = multiply2D( v_step2x , v_step2x , 'fourier2real' );                  % f_x * f_x in physical space (pseudospectral)
                    w1s_r = multiply2D( v_step2y , v_step2y , 'fourier2real' );                 % f_y * f_y in physical space (pseudospectral)
                    Nonlin_v1_r = (1/2) * ( w1_r + w1s_r );                                     % (1/2)*(f_x * f_x + f_y * f_y) in physical space 
                    Nonlin_v1 = multiply2D( fft2(Nonlin_v1_r) , fft2(Nonlin_v1_r) ,'dealias');  % dealias
                    ...
                    v_1p = ( 1 + (dt * alpha_I(k) * Lin) ).^(-1) .* ...
                        ( ( 1 - (dt * beta_I(k) * Lin) ) .* v_stepp - ...
                        (dt * alpha_E(k) * Nonlin_v1) - (dt * beta_E(k) * Nonlin_v0p) );

                    v_stepp = v_1p;
                    Nonlin_v0p = Nonlin_v1;
                end
