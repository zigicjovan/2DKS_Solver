...
enum SolutionType {
    ...
    SolveInitialState,               
    SolveTerminalState,  
    // setSolutionInTime: load data, check CFL, Nonlinear dealiasing, IMEX RK4, save {fwdALL (optimizer)}, save {energy,spectrum (last time)}, load {fwd (bwd)}  
    SolveForwardInTime,       
    SolveBackwardInTime, 
    // optimizeSolution: RCG, brent, save {optIC, branch, diags, brent} 
