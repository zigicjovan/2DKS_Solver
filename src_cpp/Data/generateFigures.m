...
for fi = 1:numel(forwardFiles)
    ...
    % Remove unused zero states
    states(:,all(states == 0,1)) = [];

    allStates = [allStates,states];
end

numberOfStates = size(allStates,2);
%}
...
for fi = 1:numel(forwardFiles)
    filename = fullfile( ...
        forwardFiles(fi).folder,forwardFiles(fi).name);
    ...
    for localIndex = 1:size(fileStates,2)
        stateIndex = stateIndex + 1;

        if stateIndex > numberOfStates
            error('Forward files contain more states than spectrumData.');
        end

        u_hat = reshape( ...
            fileStates(:,localIndex),params.N1,params.N2);

        u = real(ifft2(u_hat));
        u = circshift(u,[drow,dcol]);

        [~,projcoeffs,unstablemodes] = ...
            eigenfunction_validation( ...
                u(:),params.ell1,params.N1,'full');
        ...
        if stateIndex == 1
            ...
            projcoeffmodeevolution = ...
                zeros(numberOfModes,numberOfStates + 2);

            projcoeffmodeevolution(:,1:2) = ...
                unstablemodes(:,1:2);

            % Exact physical radius of every unstable mode
            modeRadii = hypot( ...
                unstablemodes(:,1)/params.ell1, ...
                unstablemodes(:,2)/params.ell2);
        
            % Group sign changes and symmetric modes having equal radii
            [radialRadii,representativeIdx,modeToRadialGroup] = ...
                uniquetol(modeRadii,1e-10,'DataScale',1);
        
            numberOfRadialGroups = numel(radialRadii);
        
            projcoeffradialevolution = [
                radialRadii, ...
                zeros(numberOfRadialGroups,numberOfStates)
            ];
        
            representativeModes = ...
                unstablemodes(representativeIdx,1:2);
            ...
            for j = 1:size(representativeModes,1)
                if all(representativeModes(j,:) > 0) && ...
                        representativeModes(j,1) < representativeModes(j,2)
                    representativeModes(j,:) = ...
                        fliplr(representativeModes(j,:));
            ...
            modelabels = "(" ...
                + string(representativeModes(:,1)) + "," ...
                + string(representativeModes(:,2)) + ")";
        
            modecats = categorical( ...
                modelabels,modelabels,'Ordinal',true);
        elseif ~isequal( ...
                unstablemodes(:,1:2), ...
                projcoeffmodeevolution(:,1:2))
            error( ...
                'Unstable-mode ordering changed at state %d.', ...
                stateIndex);
        end

        projcoeffmodeevolution(:,stateIndex+2) = ...
            projectionWeights;

        % Sum equivalent modes into their exact radial groups
        radialWeights = accumarray( ...
            modeToRadialGroup, ...
            projectionWeights, ...
            [numberOfRadialGroups,1], ...
            @sum,0);
        
        projcoeffradialevolution(:,stateIndex+1) = ...
            radialWeights;
        %{
        for j = 1:size(unstablemodes,1)
            % Physical radial wavenumber
            radius = hypot( ...
                unstablemodes(j,1)/params.ell1, ...
                unstablemodes(j,2)/params.ell2);

            [~,idx] = min(abs( ...
                projcoeffradialevolution(:,1) - radius));

            projcoeffradialevolution(idx,stateIndex+1) = ...
                projcoeffradialevolution(idx,stateIndex+1) ...
                + projectionWeights(j);

            if stateIndex == 1
                newLabel = "(" ...
                    + string(unstablemodes(j,1)) + "," ...
                    + string(unstablemodes(j,2)) + ")";
            
                if strlength(radialModeLabels(idx)) == 0
                    radialModeLabels(idx) = newLabel;
                else
                    radialModeLabels(idx) = ...
                        radialModeLabels(idx) + ", " + newLabel;
                end
            end
        end
...
% Retain radial bins containing at least one unstable mode
%keptIndices = find(strlength(radialModeLabels) > 0);

% Sort according to physical radial-bin radius
%[~,order] = sort(projcoeffradialevolution(keptIndices,1));
%keptIndices = keptIndices(order);
%projcoeffradialcut = projcoeffradialevolution(keptIndices,:);
%modelabels = radialModeLabels(keptIndices);
...
for fi = 1:numel(forwardFiles)
    ...
    numberOfStates = floor(numel(fwdField) / gridSize);
    for i = 1:numberOfStates
        stateIndex = stateIndex + 1;
        ...
        if stateIndex == 1
            ...
            h_proj = bar( ...
                ax(k), ...
                xmodelabels, ...
                projcoeffradialcut(:,stateIndex+1), ...
                'BarWidth',1.0);
            ...
            projcoeff_xpts = ...
                currentT*ones(numberOfRadialGroups,1);
            ...
            k = 7;
            set(proj_coeffpts, ...
                'XData',currentT*ones(numberOfRadialGroups,1), ...
                'YData',projcoeffradialcut(:,stateIndex+1));
        end    
