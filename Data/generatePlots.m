...
%% ========================================================================
%  1b. Compensated ENERGY SCALING WITH INITIAL ENERGY K
%     gnuplot: plotK.txt
%
%     Fit:
%         log10(K*) = b + a log10(K)
...
for i = 1:numel(ells)
    ...
    Kfit = K(valid);
    KstarFit = Kstar(valid);
    KstarComp = KstarFit ./ Kfit.^2;

    % Equivalent to:
    % fit f(x) ... using (log10($1)):(log10($7))
    p = polyfit(log10(Kfit), log10(KstarComp), 1);
    ...
    loglog( ...
        ...
        'MarkerSize', 6, ...
        'DisplayName', sprintf( ...
            '$\\widetilde K_{%.2f}/K^2 \\approx 10^{%.3f} K^{%.3f}$ \\,', ...
            ell, bK(i), aK(i)));
end
...
ylabel('Compensated $\widetilde{K}_{\ell} / K^2$', ...
    'Interpreter', 'latex', ...
    'FontSize', fontSize);
...
subtitle(sprintf([ ...
    '$K=[10^4,\\ldots,10^6]$, ' ...
    '$\\ell=[1.05,\\ldots,1.95]$: ' ...
    '$\\widetilde{K}_{\\ell} / K^2 \\sim K^{%.4f \\pm %.4f}$'], ...
    powerlawK, stdevK), ...
    'Interpreter', 'latex', ...
    'FontSize', fontSize);
...
exportgraphics(gcf, 'powerlawK_comp.pdf', 'ContentType', 'vector');
...
for i = 1:numel(K_values)
    ...
    semilogy( ...
        ...
        'MarkerSize', 6, ...
        'DisplayName', sprintf( ...
            '$\\widetilde K_{%s} \\approx 10^{%.3f}10^{%.3f\\ell^2}$', ...
...
subtitle(sprintf([ ...
    '$K=[10^4,10^{4.5},\\ldots,10^6]$ and ' ...
    '$\\ell=[1.05,1.20,\\ldots,1.95]$: ' ...
    '$\\widetilde{K}_{K} \\sim 10^{%.4f\\ell^2 \\pm %.4f}$'], ...
    powerlawL, stdevL), ...
    'Interpreter', 'latex', ...
    'FontSize', fontSize);
...
%% ========================================================================
%  2b. ENERGY SCALING WITH DOMAIN FACTOR ell
%     gnuplot: plotL.txt
%
%     Fit:
%         log10(K*) = b + a log10(ell)
%     so:
%         K* ~ 10^(a ell + b)
% ========================================================================
...
for i = 1:numel(K_values)
    ...
    % Equivalent to:
    % fit f(x) ... using 2:(log10($7))
    p = polyfit(log10(ellFit), log10(KstarFit), 1);
    ...
    semilogy( ...
        ...
        'MarkerSize', 6, ...
        'DisplayName', sprintf( ...
            '$\\widetilde K_{%s} \\approx 10^{%.3f} \\ell^{2(%.3f)}$', ...
            K_labels{i}, bL(i), aL(i)));
end
...
subtitle(sprintf([ ...
    '$K=[10^4,10^{4.5},\\ldots,10^6]$ and ' ...
    '$\\ell=[1.05,1.20,\\ldots,1.95]$: ' ...
    '$\\widetilde{K}_{K} \\sim \\ell^{2(%.4f \\pm %.4f)}$'], ...
    powerlawL, stdevL), ...
    'Interpreter', 'latex', ...
    'FontSize', fontSize);
...
exportgraphics(gcf, 'powerlawL_b.pdf', 'ContentType', 'vector');
...
%% ========================================================================
%  2c. Compensated ENERGY SCALING WITH DOMAIN FACTOR ell
%     gnuplot: plotL.txt
%
%     Fit:
%         log10(K*) = b + a log10(ell)
...
for i = 1:numel(K_values)
    ...
    ellFit = ell(valid);
    KstarFit = Kstar(valid);
    KstarComp = KstarFit ./ ellFit;

    % Equivalent to:
    % fit f(x) ... using 2:(log10($7))
    p = polyfit(log10(ellFit), log10(KstarComp), 1);
    ...
    semilogy( ...
        ...
        'MarkerSize', 6, ...
        'DisplayName', sprintf( ...
            '$\\widetilde K_{%s}/\\ell^2 \\approx 10^{%.3f} \\ell^{2(%.3f)}$', ...
            K_labels{i}, bL(i), aL(i)));
end
...
ylabel('Compensated $\widetilde{K}_{K} / \ell^2$', ...
    'Interpreter', 'latex', ...
    'FontSize', fontSize);
...
subtitle(sprintf([ ...
    '$K=[10^4,\\ldots,10^6]$, ' ...
    '$\\ell=[1.05,\\ldots,1.95]$: ' ...
    '$\\widetilde{K}_{K} / \\ell^2 \\sim \\ell^{2(%.4f \\pm %.4f)}$'], ...
    powerlawL, stdevL), ...
    'Interpreter', 'latex', ...
    'FontSize', fontSize);
...
exportgraphics(gcf, 'powerlawL_comp.pdf', 'ContentType', 'vector');
...
%% ========================================================================
%  0. Universal ENERGY SCALING
%     Fit:
%         log10(K*) = b + a log10(K/ell_1*ell_2)
% ========================================================================
...
for i = 1:numel(ells)
    ...
    loglog( ...
        ...
        'MarkerSize', 6, ...
        'DisplayName', sprintf( ...
            '$\\widetilde K_{%.2f}/\\ell^2 \\approx 10^{%.3f} K^{%.3f}$ \\,', ...
            ell, bK(i), aK(i)));
end
...
stdevK = std(aK, 0);   % sample standard deviation, denominator N-1
powerlawbK = mean(bK);
stdevbK = std(bK, 0);   % sample standard deviation, denominator N-1

xlabel('Initial Energy $K$', ...
    'Interpreter', 'latex', ...
    'FontSize', fontSize);

ylabel('Normalized Energy $\widetilde{K}_{\ell} / \ell^2$', ...
    'Interpreter', 'latex', ...
    'FontSize', fontSize);
...
subtitle(sprintf([ ...
    '$K=[10^4,\\ldots,10^6]$, ' ...
    '$\\ell=[1.05,\\ldots,1.95]$: ' ...
    '$\\widetilde{K}_{\\ell} / \\ell^2 \\approx 10^{%.4f} K^{%.4f}$'], ...
    powerlawbK, powerlawK), ...
    'Interpreter', 'latex', ...
    'FontSize', fontSize);
...
exportgraphics(gcf, 'powerlaw_universal.pdf', 'ContentType', 'vector');
...
for i = 1:numel(K_values)
    ...
    semilogy( ...
        ...
        'MarkerSize', 6, ...
        'DisplayName', sprintf( ...
            '$\\widetilde{T}_{%s} \\approx 10^{%.3f}10^{%.3f\\ell^2}$', ...
...
subtitle(sprintf([ ...
    '$K=[10^4,10^{4.5},\\ldots,10^6]$ and ' ...
    '$\\ell=[1.05,1.20,\\ldots,1.95]$: ' ...
    '$\\widetilde{T}_{K} \\sim 10^{%.4f\\ell^2 \\pm %.4f}$'], ...
    powerlawTL, stdevTL), ...
    'Interpreter', 'latex', ...
    'FontSize', fontSize);
...
%% ========================================================================
%  4b. TIME SCALING WITH DOMAIN FACTOR ell
%     gnuplot: plotTL.txt
%
%     Fit:
%         log10(T*) = b + a ell
...
for i = 1:numel(K_values)
    ...
    % Equivalent to:
    % fit f(x) ... using 2:(log10($4))
    p = polyfit(log10(ellFit), log10(TstarFit), 1);
    ...
    semilogy( ...
        ...
        'MarkerSize', 6, ...
        'DisplayName', sprintf( ...
            '$\\widetilde{T}_{%s} \\approx 10^{%.3f}\\ell^{2(%.3f)}$', ...
            K_labels{i}, bTL(i), aTL(i)));
...
subtitle(sprintf([ ...
    '$K=[10^4,10^{4.5},\\ldots,10^6]$ and ' ...
    '$\\ell=[1.05,1.20,\\ldots,1.95]$: ' ...
    '$\\widetilde{T}_{K} \\sim \\ell^{2(%.4f \\pm %.4f)}$'], ...
    powerlawTL, stdevTL), ...
    'Interpreter', 'latex', ...
    'FontSize', fontSize);
...
exportgraphics(gcf, 'powerlawTL_b.pdf', 'ContentType', 'vector');
