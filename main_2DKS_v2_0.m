function main_2DKS_v2_0
    ...
    timewindow = linspace(0,120,7); % for time-stepping analysis
    timewindow(1) = 10^(-3);
    ...
    L_s2 = L_scale(2); % length-scale parameter in dim 2
    dt = timestep(5); % length of time-step
    T = timewindow(7); % time window
    N = gridsize(3); % number of grid points 
    save_each = 200; % number of iterations between saved timepoints
